...
function buildTasks(options) {
    ...
    return Object.entries(localeDefinitions)
        ...
        .map(([locale, definition]) => ({
            locale,
            source: `src/data/about/About${definition.suffix}.ts`,
            getValue: () => ({
                about: require(`../src/data/about/About${definition.suffix}`)[`About${definition.suffix}`],
                media: {
                    mainImage: ImageUrl.about,
                },
            }),
        }));
}
...
async function migrate() {
    ...
    const collectionName = process.env.MONGODB_COLLECTION_ABOUT_CONTENT || 'about_content';
    ...
    try {
        for (const task of tasks) {
            const key = `${task.source}::about::${task.locale}`;

            try {
                const payload = task.getValue();

                if (!payload.about || !payload.about.description?.length || !payload.media?.mainImage) {
                    throw new Error('Resolved about content is incomplete');
                }
                ...
                if (!savedDocument || !savedDocument.about || !savedDocument.media) {
                    throw new Error('Verification failed after Mongo upsert');
                }

                appendLog(
                    successLogPath,
                    `${timestamp} | OK | key=${key} | section=about | locale=${task.locale} | source=${task.source} | target=${collectionName}`
                );
                migratedCount += 1;
            } catch (error) {
                ...
                appendLog(
                    failureLogPath,
                    `${nowIso()} | FAIL | key=${key} | section=about | locale=${task.locale} | source=${task.source} | target=${collectionName} | error=${message}`
                );
                failedCount += 1;
            }
        }
...
migrate().catch((error) => {
    appendLog(
        failureLogPath,
        `${nowIso()} | FAIL | key=process | section=about | locale=all | source=scripts/migrate-about-to-mongo.cjs | error=${error.message}`
    );
    console.error(error);
    process.exit(1);
});
